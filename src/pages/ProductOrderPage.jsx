...
export default function ProductOrderPage() {
  ...
  const loadProducts = useCallback(async () => {
    ...
    const orderableProducts = sourceProducts.filter((product) => product.order_enabled !== false)
  ...
  const submitOrder = async (event) => {
    ...
    try {
      ...
      try {
        ...
        const saved = JSON.parse(window.localStorage.getItem('ruicheng_public_orders') || '[]')
        window.localStorage.setItem('ruicheng_public_orders', JSON.stringify([...saved, { ...payload, created_at: new Date().toISOString() }]))
      }
      setSubmitted(true)
      setQuantity(1)
