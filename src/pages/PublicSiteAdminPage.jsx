...
export default function PublicSiteAdminPage() {
  ...
  const [status, setStatus] = useState(() => (
    window.localStorage.getItem(PUBLIC_SITE_PENDING_KEY) === '1'
      ? '目前瀏覽器有尚未發布到外部 API 的草稿。若尚未設定 API，這份內容會先保留在目前瀏覽器。'
      : ''
  ))
  ...
  const guardedLock = () => {
    ...
    window.localStorage.removeItem('ruicheng_public_cms_unlocked')
  ...
  const save = async (nextDraft = draft) => {
    try {
      ...
      setJsonText(JSON.stringify(serverContent, null, 2))
      setSavedSnapshot(JSON.stringify(serverContent))
      setStatus('已儲存公開官網內容到外部 API。Safari 與其他瀏覽器重新整理後也會看到同一版。')
    } catch (error) {
      const statusCode = error?.response?.status
      const nextMessage = statusCode === 401 || statusCode === 403
        ? '外部 API 儲存失敗：CMS token 不正確或沒有權限。已先儲存在目前瀏覽器。'
        : '外部 API 尚未連線或儲存失敗，已先儲存在目前瀏覽器。若要讓 Vercel 也同步，請匯出 JSON 後更新預設內容，或設定正式 CMS API。'
  ...
  const reset = () => {
    ...
    resetPublicSiteContent()
    window.localStorage.removeItem(PUBLIC_SITE_PENDING_KEY)
    setDraft(bundledPublicSiteContent)
    setJsonText(JSON.stringify(bundledPublicSiteContent, null, 2))
    setSavedSnapshot(JSON.stringify(bundledPublicSiteContent))
    setStatus('已恢復為目前打包進 Vercel 的官網內容。')
  }
    ...
    <main className="public-site-admin">
      <header className="public-site-admin__bar">
        ...
        <nav>
          ...
          <Link to="/login" onClick={guardNavigation}>內部平台入口</Link>
          <button type="button" onClick={guardedLock}>鎖定 CMS</button>
        </nav>
      </header>

      <section className="admin-notice">
        <strong>獨立官網 CMS</strong>
        {isDirty && <strong className="dirty-indicator">尚未儲存變更</strong>}
        <span>
          這個後台只編輯公開官網內容，不會出現在公開網站導覽列。請只放對外可公開的概念性內容，不要放入材料參數、STL
          版本、BOM 成本、報告或稽核紀錄；那些資料應留在朋友維護的內部雲端平台。
        </span>
      </section>
      ...
      <section className="admin-grid">
          ...
          {activeTab === 'catalog' && (
            <>
              ...
              <EditorPanel title="產品頁面文字">
                <p className="admin-helper">
                  這裡修改的是 /catalog 產品目錄的公開頁面文字。產品項目由內建 JSON 或朋友平台的公開產品 API 提供，不在官網 CMS 直接維護內部產品資料。
                </p>
          ...
          {activeTab === 'login' && (
            <>
              ...
              <EditorPanel title="登入頁文字">
                <p className="admin-helper">
                  這裡只修改 /login 的入口頁文案。真正登入驗證、帳號密碼與高權限判斷由獨立的內部雲端平台負責。
                </p>
                <div className="admin-actions">
        ...
        <aside className="admin-save-panel">
          <h2>發布控制</h2>
          <p>儲存時會先嘗試同步到外部 CMS API；若尚未設定 API，會先存在目前瀏覽器。正式部署前可把 JSON 匯出後打包進 Vercel，或接上正式 CMS API。</p>
