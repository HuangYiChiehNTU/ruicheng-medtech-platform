...
import exportedPublicSiteContent from './exportedPublicSiteContent.json'
...
export const bundledPublicSiteContent = mergeContent(
  defaultPublicSiteContent,
  exportedPublicSiteContent?.content || exportedPublicSiteContent || {}
)

export function getPublicSiteContent() {
  if (typeof window === 'undefined') return bundledPublicSiteContent

  try {
    const saved = window.localStorage.getItem(PUBLIC_SITE_CONTENT_KEY)
    return saved ? mergeContent(bundledPublicSiteContent, JSON.parse(saved)) : bundledPublicSiteContent
  } catch {
    return bundledPublicSiteContent
  }
}

export async function fetchPublicSiteContent() {
  const { data } = await api.get('/public-site-content')
  return data.content && Object.keys(data.content).length > 0
    ? mergeContent(bundledPublicSiteContent, data.content)
...
export async function savePublicSiteContentToServer(content) {
  ...
  return mergeContent(bundledPublicSiteContent, data.content)
}
