...
export default function ProjectMembersPanel({ projectId }) {
  ...
  return (
    <div className="members-panel-grid">
      <form onSubmit={addMember} style={{ display: 'grid', alignContent: 'start', gap: 10, padding: 14, borderRadius: 8, background: '#0f172a', border: '1px solid #334155' }}>
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ display: 'grid', gap: 8 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1.5fr 0.9fr 1fr 1fr auto', gap: 10, padding: '0 10px', color: '#94a3b8', fontSize: 12 }}>
            <span>姓名</span>
            <span>Email</span>
            <span>角色</span>
            <span>專案權限</span>
          ...
          {members.map((member) => (
            <div key={member.mapping_id} style={{ display: 'grid', gridTemplateColumns: '1.2fr 1.5fr 0.9fr 1fr 1fr auto', alignItems: 'center', gap: 10, fontSize: 12, padding: 10, borderRadius: 8, background: '#0f172a', border: '1px solid #334155' }}>
              <strong>{member.name}</strong>
              <span style={{ color: '#cbd5e1', overflowWrap: 'anywhere' }}>{member.email}</span>
              <span style={{ color: '#94a3b8' }}>{ROLE_LABELS[member.role] || member.role}</span>
              ...
              <span style={{ color: '#94a3b8' }}>{member.last_activity_at ? new Date(member.last_activity_at).toLocaleString() : '尚無紀錄'}</span>
              <button onClick={() => removeMember(member)} style={{ ...buttonStyle, background: '#7f1d1d', padding: '6px 10px' }}>移除</button>
            </div>
          ))}
        </div>
      </div>
...
const buttonStyle = { border: 'none', background: '#3b82f6', color: '#fff', borderRadius: 6, padding: '8px 12px', cursor: 'pointer', fontWeight: 700 }
